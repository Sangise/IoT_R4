 ...
 * this function. 
 * *********************************************************/
int mcp3208Spi::spiOpen(std::string devspi){
    int statusVal = -1;
 ...
 * "data". 
 * ******************************************************************/
int mcp3208Spi::spiWriteRead( unsigned char *data, int length){
...
//one spi transfer for each byte
 for (i = 0 ; i < length ; i++){
	...
	memset(&spi[i], 0, sizeof(spi[length]));
	
    spi[i].tx_buf
        = (unsigned long)(data + i); //transmit from "data"
    spi[i].rx_buf
        = (unsigned long)(data + i) ; //receive into "data"
    spi[i].len
           = sizeof(*(data + i)) ;
    spi[i].delay_usecs
   = 0 ; 
 ...
 * ***********************************************/
mcp3208Spi::mcp3208Spi(std::string devspi, unsigned char spiMode, unsigned int spiSpeed, unsigned char spibitsPerWord){
    this->mode = spiMode ; 
